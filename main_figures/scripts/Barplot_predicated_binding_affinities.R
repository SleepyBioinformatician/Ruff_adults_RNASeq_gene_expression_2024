...
data <- data.frame(
  ...
  values = c(9.47, 8.93, 10.07, 9.30, 9.81, 9.21)
)
...
p <- ggplot(data, aes(x = values, 
  ...
  geom_vline(xintercept = 10,  col = "darkgrey", linewidth = 0.25, lty = 2) +
  ...
  coord_cartesian(xlim = c(6.5, 10.5)) +
  labs(title = "", 
       x = expression("Predicted binding affinity"~Delta*G~"Binding (kcal mol"^"-1"*")"),  
       y = "") +
